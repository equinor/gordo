...
def test_normalize_sensor_tag_from_dict():
    tag_list_as_list_of_dict = [
        {"name": TAG_NAME1, "asset": asset_nonsense},
        {"name": TAG_NAME2, "asset": asset_nonsense},
    ]
    tag_list_as_list_of_sensor_tag = [
        normalize_sensor_tag(tag_dict) for tag_dict in tag_list_as_list_of_dict
    ]
...
def test_normalize_sensor_tag_from_string():
    with pytest.raises(ValueError):
        tag_list_as_list_of_strings_nonsense = [TAG_NAME1, TAG_NAME2]
        [
            normalize_sensor_tag(tag_string)
            for tag_string in tag_list_as_list_of_strings_nonsense
        ]

    tag_list_as_list_of_known_tag_strings = ["TRC-123", "GRA-214", "ASGB-212"]
    tag_list_as_list_of_sensor_tag = [
        normalize_sensor_tag(tag_string)
        for tag_string in tag_list_as_list_of_known_tag_strings
    ]
...
def test_normalize_sensor_tag_from_sensor_tag():
    ...
    normalized_sensor_tags = [
        normalize_sensor_tag(sensor_tag) for sensor_tag in sensor_tags
    ]
