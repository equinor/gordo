...
def dataset_config(mock_file_system, mock_assets_config):
    ...
    return {
        ...
        "data_provider": DataLakeProvider(storage=mock_file_system, assets_config=mock_assets_config),
    }


def test_get_data_serviceauth_fail(caplog, dataset_config, mock_file_system, mock_assets_config):
    train_start_date = dateutil.parser.isoparse("2017-01-01T08:56:00+00:00")
    train_end_date = dateutil.parser.isoparse("2017-01-01T10:01:00+00:00")
    ...
    dataset_config["data_provider"] = DataLakeProvider(
        storage=mock_file_system, assets_config=mock_assets_config, dl_service_auth_str="TENTANT_UNKNOWN:BOGUS:PASSWORD"
    )
...
def test_get_data_serviceauth_in_config(mock_file_system, mock_assets_config, default_config):
    dataset_config = default_config
    dataset_config["data_provider"] = DataLakeProvider(
        storage=mock_file_system, assets_config=mock_assets_config, dl_service_auth_str=os.getenv("TEST_SERVICE_AUTH")
    )
