...
def watchman(
    ...
    with responses.RequestsMock(
    ...
    ) as rsps, temp_env_vars(
        MODEL_LOCATION=model_location
        # For now we need to patch the async fetch_json with sync version until we know how to mock it properly
    ), async_mock.patch(
        ...
        rsps.add_callback(
            responses.GET,
            re.compile(rf".*{namespace}.ambassador.*\/gordo\/v0\/{project}\/.*.\/.*."),
