...
class PredictionInfluxTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        influx = cls.docker_client.containers.run(image='influxdb:1.7-alpine',
                                                  environment={
        ...
        cls._seed_source_db()
        cls.influx_config = {'host': 'localhost', 'port': 8087,
    ...
    def _load_unique_sensors():
        ...
        config = os.path.join(path_to_config,'config-influx-test.yaml')
    ...
    def _seed_source_db():
        ...
        client = InfluxDBClient(
            'localhost', 8087, 'root', 'root', SOURCE_DB_NAME, proxies={'http': '', 'https': ''})
        dates = pd.date_range(start='2016-01-01', periods=1440, freq='min')  # Minute intervals for 1 day
    ...
    def test_read_single_sensor_empty_data_time_range_indexerror(self):
        ...
        ds = InfluxBackedDataset(influx_config=self.influx_config,from_ts=from_ts,to_ts=to_ts)
    ...
    def test__list_of_tags_from_influx_validate_tag_names(self):
        ...
        to_ts = dateutil.parser.isoparse(to_ts)
        ds = InfluxBackedDataset(influx_config=self.influx_config, from_ts=from_ts, to_ts=to_ts)
        expected_tags = {'TRC-FIQ -23-0453N','TRC-FIQ -80-0303N',
