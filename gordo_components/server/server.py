...
class PredictionApiView(MethodView):
    ...
    def post(self):
        ...
        if X.dtype == np.dtype('O'):
            return jsonify({'error': 'Either provided non numerical elements or records with different shapes.'
                                     '  ie. [[0, 1, 2], [0, 1]]'}), 400
        ...
        # exceptions twice if it happens.
            ...
            try:
                ...
                context['error'] = "Something unexpected happened; please check your input data"
...
def build_app():
    ...
    app.add_url_rule(rule='/predictions',
                     view_func=PredictionApiView.as_view("predictions"),
                     methods=['POST'])

    app.add_url_rule(rule='/metadata',
                     view_func=PredictionApiView.as_view("metadata"),
                     methods=['GET'])

    app.add_url_rule(rule='/healthcheck',
